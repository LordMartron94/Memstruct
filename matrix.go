...
// MatrixShiftRight shifts a contiguous range of elements in the matrix
...
//
// Parameters:
//   - fromRow, fromCol:  starting position of the range to shift (inclusive)
//   - toRow, toCol:      ending position of the range to shift (inclusive)
//   - count:             number of positions to shift the range to the right
//
//go:nosplit
//go:inline
func MatrixShiftRight[T foundation.Numeric](matrix memcore.MarkRaw, fromRow, fromCol, toRow, toCol, count uint64) error {
	matrixPtr := memcore.MemcoreMarkDereferenceObjectUnsafe[Matrix[T]](matrix)

	fromIdx := matrixRowColToIdx(fromRow, fromCol, matrixPtr.cols)
	toIdx := matrixRowColToIdx(toRow, toCol, matrixPtr.cols)

	return VectorShiftRight[T](matrixPtr.data, fromIdx, toIdx, count)
}

// MatrixShiftRightUnsafe shifts a contiguous range of elements in the matrix
...
//go:nosplit
//go:inline
func MatrixShiftRightUnsafe[T foundation.Numeric](matrix memcore.MarkRaw, fromRow, fromCol, toRow, toCol, count uint64) {
	matrixPtr := memcore.MemcoreMarkDereferenceObjectUnsafe[Matrix[T]](matrix)

	fromIdx := matrixRowColToIdx(fromRow, fromCol, matrixPtr.cols)
	toIdx := matrixRowColToIdx(toRow, toCol, matrixPtr.cols)

	VectorShiftRightUnsafe[T](matrixPtr.data, fromIdx, toIdx, count)
}

// MatrixShiftLeft shifts a contiguous range of elements in the matrix
...
//go:nosplit
//go:inline
func MatrixShiftLeft[T foundation.Numeric](matrix memcore.MarkRaw, fromRow, fromCol, toRow, toCol, count uint64) error {
	matrixPtr := memcore.MemcoreMarkDereferenceObjectUnsafe[Matrix[T]](matrix)

	fromIdx := matrixRowColToIdx(fromRow, fromCol, matrixPtr.cols)
	toIdx := matrixRowColToIdx(toRow, toCol, matrixPtr.cols)

	return VectorShiftLeft[T](matrixPtr.data, fromIdx, toIdx, count)
}

// MatrixShiftLeftUnsafe shifts a contiguous range of elements in the matrix
...
//go:nosplit
//go:inline
func MatrixShiftLeftUnsafe[T foundation.Numeric](matrix memcore.MarkRaw, fromRow, fromCol, toRow, toCol, count uint64) {
	matrixPtr := memcore.MemcoreMarkDereferenceObjectUnsafe[Matrix[T]](matrix)

	fromIdx := matrixRowColToIdx(fromRow, fromCol, matrixPtr.cols)
	toIdx := matrixRowColToIdx(toRow, toCol, matrixPtr.cols)

	VectorShiftLeftUnsafe[T](matrixPtr.data, fromIdx, toIdx, count)
}
